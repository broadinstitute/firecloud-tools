...
def fix_outputs(ws_namespace, ws_name, submission_id, expressions_override):
    # get the workspace
    workspace_request = api.get_workspace(ws_namespace, ws_name, fc_url)
    ...
    # get the submission
    submission_request = api.get_submission(ws_namespace, ws_name, submission_id, fc_url)
    ...
    with open(tsv_filename, 'wb') as entity_update_tsv:
        ...
        method_config_json = api.get_workspace_config(ws_namespace, ws_name, mc_namespace, mc_name).json()
        ...
        for workflow_json in submission_workflows_json:
            workflow_status = workflow_json["status"]
            # skip workflows that did not succeed
    ...
    if upload:
        ...
        upload_request = api.upload_entities_tsv(ws_namespace, ws_name, tsv_filename)
...
def get_workflow_metadata(namespace, name, submission_id, workflow_id):
    headers = api._fiss_access_headers()
    uri = "{0}/workspaces/{1}/{2}/submissions/{3}/workflows/{4}".format(
        fc_url, namespace, name, submission_id, workflow_id)
...
fc_url = "https://api.firecloud.org/api"
def main():
    global fc_url
    
    # The main argument parser
    parser = ArgumentParser(description="Clean up files from a given submission.")
    ...
    print "        * The output expressions must all be of the form 'this.attribute_name' - this does not handle cases such as 'this.case_sample.attribute_name'"
    print "        * The root entity type chosen has to be either a single entity of root entity type or a set of those entities.  This will not work for instance if your method runs on a sample and you chose a pair with the launch expression 'this.case_sample'."
